...
TRAINING_ITEMS = {
    "フォークリフト安全講習": {
        "sections": [
            ("① 始業前点検・使用停止判断", [
                ("始業前点検：タイヤ損傷・摩耗確認不足", 3),
                ("始業前点検：タイヤ空気圧確認不足", 3),
                ("始業前点検：ホイールナット確認不足", 3),
                ("始業前点検：フォーク爪の摩耗・亀裂確認不足", 3),
                ("始業前点検：フォーク爪の左右差確認不足", 3),
                ("始業前点検：マスト・チェーン確認不足", 3),
                ("始業前点検：油圧漏れ確認不足", 3),
                ("始業前点検：ブレーキ確認不足", 3),
                ("始業前点検：駐車ブレーキ確認不足", 3),
                ("始業前点検：ハンドル確認不足", 3),
    ...
    "玉掛け安全教育": {
        "sections": [
            ...
            ("④ 吊り上げ・移動・着地", [
                ...
                ("吊り移動：荷の安定確認不足", 3),
                ("吊り移動：着地場所の耐荷重確認不足", 3),
                ("吊り移動：玉掛け用具取り外し時の安定確認不足", 3),
                ("吊り移動：吊り荷に手を挟む位置で作業", 3),
            ]),
            ("⑤ 終了・記録・改善", [
                ("終了記録：用具片付け不足", 3),
                ("終了記録：用具損傷報告不足", 3),
    ...
    "クレーン安全教育": {
        "sections": [
            ...
            ("③ 吊り荷移動・着地", [
                ...
                ("吊り荷移動：長尺物の振れ確認不足", 3),
                ("吊り荷移動：着地場所確認不足", 3),
                ("吊り荷移動：手元作業者との距離確認不足", 3),
                ("吊り荷移動：狭所での退避位置確認不足", 3),
            ]),
            ("④ 共同作業・立入管理", [
                ("共同作業：合図者不明確", 3),
                ("共同作業：合図方法不統一", 3),
    ...
    "KY活動": {
        "sections": [
            ...
            ("② 危険の洗い出し", [
                ...
                ("危険抽出：飛来落下リスクの見落とし", 5),
                ("危険抽出：感電リスクの見落とし", 1),
                ("危険抽出：火気・火災リスクの見落とし", 3),
                ("危険抽出：転倒リスクの見落とし", 5),
                ("危険抽出：熱中症・寒冷リスクの見落とし", 1),
                ("危険抽出：有害物・粉じんリスクの見落とし", 1),
            ]),
            ("③ 対策の具体性", [
                ("対策：対策が抽象的", 1),
                ("対策：誰が何をするか不明確", 3),
    ...
    "新入社員安全教育": {
        "sections": [
            ("① 基本ルール・報連相", [
                ("基本ルール：会社安全ルール理解不足", 3),
                ("基本ルール：報連相理解不足", 3),
                ("基本ルール：勝手な判断で作業しようとした", 1),
                ("基本ルール：分からない時に確認しない", 1),
                ("基本ルール：立入禁止区域理解不足", 5),
                ("基本ルール：安全通路理解不足", 3),
                ("基本ルール：作業指示確認不足", 3),
                ("基本ルール：作業前KY理解不足", 3),
...
def generate_ai_guidance(training_type, score, deduction, result, checked_items, fail_items, memo):
    """減点項目から指導メモ用の総評を自動生成する簡易AI風ロジック。"""
    if fail_items:
        headline = "一発失格項目があるため、作業再開前に再教育が必要です。"
    elif score >= 90:
        headline = "全体として安全意識は良好です。基本動作を継続してください。"
    elif score >= 70:
        headline = "合格水準ですが、重点項目の再確認が必要です。"
    else:
        headline = "安全上の不安が残るため、再教育を推奨します。"
    parsed = []
    for item in checked_items:
        m = re.match(r"−(\d+)点\s+(.+)", item)
        parsed.append((int(m.group(1)) if m else 0, m.group(2) if m else item))
    parsed.sort(reverse=True, key=lambda x: x[0])
    top_items = [x[1] for x in parsed[:5]]
    joined = " ".join(top_items + fail_items)
    focus = []
    if any(w in joined for w in ["後進", "死角", "歩行者", "第三者", "一般車両"]): focus.append("周囲確認・第三者災害防止")
    if any(w in joined for w in ["合図", "声かけ", "誘導", "無線"]): focus.append("合図・声かけ・連携")
    if any(w in joined for w in ["急", "速度", "徐行", "一時停止", "走行"]): focus.append("速度管理・一時停止")
    if any(w in joined for w in ["点検", "異常", "使用停止", "ブレーキ", "油圧"]): focus.append("始業前点検・異常時停止")
    if any(w in joined for w in ["荷", "吊り", "積込", "過積載", "偏荷重"]): focus.append("荷扱い・安定確認")
    if not focus: focus.append("基本ルールの再確認")
    lines = [f"【AI総評】{training_type}：{headline}", f"判定：{result}／点数：{score}点／減点合計：{deduction}点"]
    lines.append("重点指導項目：" + ("、".join(top_items) if top_items else "大きな減点はありません。"))
    lines.append("指導方針：" + "、".join(dict.fromkeys(focus)) + "を重点的に再確認してください。")
    if fail_items: lines.append("一発失格項目：" + "、".join(fail_items) + "。責任者立会いで再教育を実施してください。")
    if memo and str(memo).strip(): lines.append("講師メモ：" + str(memo).strip())
    return "\n".join(lines)
